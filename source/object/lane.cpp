...
Lane::Lane(TextureHolder* holder, Vec2 size, int pos, int len, float speed, int spawnClock, bool isRed, int trafficClock) : gridSize(size),
                                                                                                                            holder(holder),
                                                                                                                            pos(pos),
                                                                                                                            length(len),
                                                                                                                            grassType(rand() % 3),
...
void Lane::process() {
    ...
    if(clock >= spawn) {
        if(speed > 0)
            addVehicle(float(-1.0f));
        else
            addVehicle(float(length + 1.0f));
        spawn = random(60, 75);
        clock = 0;
    }
