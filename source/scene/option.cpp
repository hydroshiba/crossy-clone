...
Option::Option(Engine* engine, Speaker* speaker, SceneRegistry* registry, Setting* setting, Keyboard* keyboard, TextureHolder* holder) : Scene(engine, speaker, registry, setting, keyboard, holder),
    ...
    if(setting->volMusic() == Volume::min)
        musicId = 0;
    else if(setting->volMusic() == Volume::low)
        musicId = 1;
    else if(setting->volMusic() == Volume::medium)
        musicId = 2;
    else if(setting->volMusic() == Volume::high)
        musicId = 3;
    else
        musicId = 4;

    if(setting->volSFX() == Volume::min)
        sfxId = 0;
    else if(setting->volSFX() == Volume::low)
        sfxId = 1;
    else if(setting->volSFX() == Volume::medium)
        sfxId = 2;
    else if(setting->volSFX() == Volume::high)
        sfxId = 3;
    else
        sfxId = 4;
